...
describe("Runtime", () => {
  describe("-> NodeJs", () => {
    ...
    beforeEach(() => {
      ...
      lodash.set(plugin, "service.provider.runtime", "nodejs20.x");
    ...
    describe("-> hasDependenciesChanges", () => {
      beforeEach(() => {
        ...
        lodash.set(plugin, "bucketService.downloadDependencesFile", () =>
          Promise.resolve(remoteDeps)
        );
      ...
      it("checks if version is compatible", () => {
        return runtimes._runtime
          .isCompatibleVersion("v12.16")
          .then((res) => {
            expect(res.isCompatible).to.equal(true);
          })
          .then(() =>
            runtimes._runtime.isCompatibleVersion("v12.18.3").then((res) => {
              expect(res.isCompatible).to.equal(true);
            })
          );
      });
  ...
  describe("-> Python", () => {
    ...
    it("should throw error when undefined runtime", () => {
      ...
      runtimes = new Runtime(plugin);
      expect(
        plugin.error.calledWith("service.provider.runtime is required!")
      ).to.equal(true);
    ...
    it("should throw error when invalid runtime", () => {
      ...
      runtimes = new Runtime(plugin);
      expect(
        plugin.log.calledWith('"invalid" runtime is not supported (yet).')
      ).to.equal(true);
      expect(process.exit.calledOnce).to.equal(true);
    });
  });
