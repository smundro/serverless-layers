...
class Runtimes {
  constructor(plugin) {
    ...
    this._runtime.isCompatibleVersion(runtime).then((data) => {
      if (!data.isCompatible) {
        ...
        this.plugin.warn(
          `WARN: The current environment and Lambda runtime don't match (current=${data.version.replace(
            "\n",
            ""
          )} vs runtime=${runtime}).`
        );
  ...
  getDefaultSettings(inboundSettings = {}) {
    if (inboundSettings.packagePath) {
      console.warn(
        'WARN You should use "dependenciesPath" instead of the deprecated "packagePath" param.'
      );
